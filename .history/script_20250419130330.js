...
function fillChannelContent(contentEl, slug) {
  ...
  fetch(`https://api.are.na/v2/channels/${slug}?per=100`)
    ...
    .then(data => {
      ...
      // Create the grid based on content
      if (data.metadata?.description) {
      ...
      // Ensure content grid is updated and visible
...
function toggleContent(id, slug) {
  ...
  if (isOpen) {
    ...
    // Ensure content is loaded only once
    if (!loadedChannels.has(slug)) {
...
function createChannelItem(channelData, index) {
  ...
  contentItem.style.display = 'none';  // Content should be hidden by default
