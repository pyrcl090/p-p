...
fetch('https://api.are.na/v2/channels/rusalka?per=100')
  ...
  .then(data => {
    ...
    const cols = 15;
    const cellSize = 30;
    let rows = 60; // Start with 60 rows, can expand
    ...
    // 2. Place content inline with tracking
    let currentRow = 0;
    let currentCol = 0;
    ...
    blocks.forEach(block => {
      ...
      if (block.image) {
        ...
        while (!placed) {
          if (isFree(currentRow, currentCol, rowSpan, colSpan)) {
      ...
      text.split('').forEach(char => {
        while (!isFree(currentRow, currentCol)) {
          advance();
        }

        const charDiv = document.createElement('div');
        charDiv.className = 'overlay-item';
        charDiv.textContent = char;
        charDiv.style.gridColumn = `${currentCol + 1}`;
        charDiv.style.gridRow = `${currentRow + 1}`;

        overlayGrid.appendChild(charDiv);
