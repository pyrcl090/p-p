...
function placeChannel(channelData, index) {
  const wrapper = document.createElement('div');
  wrapper.className = 'channel-block';
  wrapper.style.display = 'contents'; // let children be part of grid
  ...
  for (const char of title) {
    ...
    item.addEventListener('click', () => toggleChannel(toggleId, channelData.slug));
    wrapper.appendChild(item);
  }

  // Add spacer to complete row if needed
  const remainder = title.length % cols;
  if (remainder !== 0) {
  ...
  // Add hidden content container
  const contentContainer = document.createElement('div');
  contentContainer.id = toggleId;
  contentContainer.className = 'channel-container';
  contentContainer.style.display = 'none';
  contentContainer.style.gridColumn = `span ${cols}`;
...
function toggleChannel(id, slug) {
  // Collapse previous
