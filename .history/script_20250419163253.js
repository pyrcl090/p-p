  ...
  fetch(`https://api.are.na/v2/channels/${slug}?per=100`)
    ...
    .then(data => {
      ...
      data.contents.forEach(block => {
        ...
        // Handle title and description as text
        const textParts = [];
        ...
        if (block.description) {
          textParts.push(block.description);
        }

        // Handle actual text block content
        if (block.class === 'Text' && (block.content_html || block.content)) {
          const rawText = block.content_html || block.content;
          const plainText = rawText.replace(/<[^>]+>/g, ''); // Strip HTML tags
          console.log('Text block content:', plainText);
