  ...
  fetch(`https://api.are.na/v2/channels/${slug}?per=100`)
    ...
    .then(data => {
      ...
      if (data.metadata?.description) {
        ...
        totalCells += Math.ceil(data.metadata.description.length / cols) * cols + cols;
      }
      ...
      data.contents.forEach(block => {
        ...
        if (text) {
          ...
          totalCells += Math.ceil(text.length / cols) * cols + cols;
        }
      ...
      const contentGrid = contentEl.previousElementSibling; // the grid
