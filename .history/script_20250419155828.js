    ...
    .then(data => {
      ...
      data.contents.forEach(block => {
        if (block.image?.display?.url) {
          ...
          // Randomly align the image to the left or right by adjusting the column span
          const randomOffset = Math.random() > 0.5 ? 0 : (cols - colSpan); // 0 for left, (cols - colSpan) for right
          img.style.gridColumnStart = randomOffset + 1;  // Place the image in either the leftmost or rightmost part of the grid
