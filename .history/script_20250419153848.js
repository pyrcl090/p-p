    ...
    .then(data => {
      ...
      data.contents.forEach(block => {
        if (block.image?.display?.url) {
          ...
          const colSpan = Math.floor(Math.random() * 6) + 3; // 3 to 8 columns
          const rowSpan = 5; // 5 rows
      ...
      // Add padding to help images not overflow
      const rowsNeeded = Math.ceil(totalCells / cols) + 2; // buffer rows
...
function createChannelItem(channelData, index) {
  ...
  const placeholderGrid = createGridStructure(200); // default placeholder
