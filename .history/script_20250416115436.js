...
const channels = [
  ...
  function toggleChannelContent(channelId) {
    ...
    if (channelContainer) {
      const currentDisplay = channelContainer.style.display;
      channelContainer.style.display = (currentDisplay === 'none' || currentDisplay === '') ? 'grid' : 'none';
    }
  
    currentlyOpenId = (currentlyOpenId === channelId) ? null : channelId;
  }
  ...
  channels.forEach((slug, index) => {
    fetch(`https://api.are.na/v2/channels/${slug}?per=100`)
      ...
      .then(data => {
        ...
        channelContainer.style.display = 'none'; // ensure hidden initially
        channelContainer.style.position = 'relative';
        channelContainer.style.zIndex = '5';
        channelContainer.style.pointerEvents = 'auto';
        ...
        data.contents.forEach(block => {
          if (block.image && block.image.display && block.image.display.url) {
            ...
            img.style.height = '100%';
            img.style.opacity = '.8';
            channelContainer.appendChild(img); // Fix: insert into channel container
            currentCol = 0;
            currentRow = row - 1 + spanRows + 1;
          }
