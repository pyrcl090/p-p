...
const cols = 15;  // Fixed number of columns
const cellSize = 30;  // Each grid cell size in pixels
let currentlyOpenId = null;
const loadedChannels = new Set();
...
function createGridStructure(numItems) {
  ...
  const rows = Math.ceil(numItems / cols);  // Calculate rows based on total items and columns
  const totalCells = rows * cols;  // Total cells to fit everything
  ...
  for (let i = 0; i < totalCells; i++) {
    ...
    // Create sub-cells within each grid cell
    for (let j = 0; j < 4; j++) {
      const sub = document.createElement('div');
      sub.className = 'sub-cell';
      cell.appendChild(sub);
...
function addTextBlock(text, container) {
  ...
  // Ensure each row has complete cells
  const remainder = text.length % cols;
  if (remainder !== 0) {
    for (let i = 0; i < cols - remainder; i++) {
      container.appendChild(document.createElement('div'));
    ...
    .then(data => {
      ...
      data.contents.forEach(block => {
        if (block.image?.display?.url) {
          ...
          const colSpan = Math.floor(Math.random() * 6) + 3; // Image should span between 3 and 8 columns
          const rowSpan = 5;  // Images will span 5 rows

          img.style.gridColumn = `span ${colSpan}`;
          img.style.gridRow = `span ${rowSpan}`;
          contentEl.appendChild(document.createElement('div')); // line break before image
          contentEl.appendChild(img);
          contentEl.appendChild(document.createElement('div')); // line break after image
...
function createChannelItem(channelData, index) {
  ...
  const grid = createGridStructure(title.length);  // Create a grid structure based on the title length
  ...
  const placeholderGrid = createGridStructure(200);  // Default placeholder grid
  ...
  contentWrapper.appendChild(placeholderGrid);
