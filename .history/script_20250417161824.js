...
//     // Fill last row with empty divs if needed
...
const channels = [
  ...
  gridContainer.style.display = 'grid';
  gridContainer.style.gridTemplateColumns = `repeat(${cols}, ${cellSize}px)`;
  gridContainer.style.gridTemplateRows = `repeat(40, ${cellSize}px)`; // Keep this fixed or make it dynamic later
  gridContainer.style.gap = '2px';
  
  // Create the background subgrid (dashed lines in cells)
  function createSubgrid(rows = 40, cols = 15) {
    for (let i = 0; i < rows * cols; i++) {
      const cell = document.createElement('div');
      cell.className = 'grid-item';
  
      // Add subcells for detailed grid lines
      for (let j = 0; j < 4; j++) {
        const sub = document.createElement('div');
        sub.className = 'sub-cell';
        cell.appendChild(sub);
  ...
  createSubgrid(); // Initialize subgrid on page load
  
  // Helper to create grid items (characters or links)
  function createGridItem(char, isLink = false, href = '') {
    const div = document.createElement('div');
    div.className = 'overlay-item';
    div.style.pointerEvents = isLink ? 'auto' : 'none';
  ...
  // Create wrapper for the channel and its content
  function createChannelContainer(channelIndex) {
    const wrapper = document.createElement('div');
    wrapper.className = 'channel-wrapper';
    wrapper.dataset.index = channelIndex;
  ...
  // Layout the channel title and its clickable toggle
  function placeChannelTitle(channelData, channelIndex) {
    const title = channelData.title || `Channel ${channelIndex + 1}`;
    const wrapper = createChannelContainer(channelIndex);
    const toggleId = `channel-content-${channelIndex}`;
  
    // Add each character of the title as a grid item
    for (const char of title) {
      const item = createGridItem(char);
      item.style.cursor = 'pointer';
      item.style.pointerEvents = 'auto';
    ...
    // Add a spacer for filling the rest of the row
    for (let i = title.length; i < cols; i++) {
      const spacer = document.createElement('div');
      wrapper.appendChild(spacer);
    }
  
    // Content container (hidden initially, inside grid)
    const contentContainer = document.createElement('div');
    contentContainer.id = toggleId;
    contentContainer.classList.add('channel-content');
    contentContainer.style.gridColumn = `1 / span ${cols}`; // Span across all columns
    contentContainer.style.display = 'none'; // Hidden initially
    contentContainer.style.gridTemplateColumns = `repeat(${cols}, ${cellSize}px)`;
    contentContainer.style.gridAutoRows = `${cellSize}px`;
    contentContainer.style.display = 'grid'; // Display as grid
    contentContainer.style.gap = '2px';
  ...
  // Add text blocks to the content container
  function placeTextBlock(text, container) {
    for (const char of text) {
      const div = createGridItem(char);
      container.appendChild(div);
    ...
    // Fill the last row with empty divs if necessary
    const remainder = text.length % cols;
    if (remainder !== 0) {
      for (let i = 0; i < cols - remainder; i++) {
        container.appendChild(document.createElement('div'));
  ...
  // Toggle a channel content block open or closed
  function toggleChannel(id, slug, wrapper) {
    // Collapse previously open content
    if (currentlyOpenId && currentlyOpenId !== id) {
      const prev = document.getElementById(currentlyOpenId);
        ...
        .then(data => {
          ...
          data.contents.forEach(block => {
            if (block.image?.display?.url) {
              ...
              img.src = block.image.display.url;
              img.className = 'grid-image';
              img.style.gridColumn = `span ${Math.floor(Math.random() * 6) + 3}`; // Random width
              img.style.gridRow = `span 5`; // Fixed height for images
              img.style.width = '100%';
              img.style.height = '100%';
              img.style.objectFit = 'cover';
              img.style.opacity = '0.85';
  ...
  // Load all channel titles (quick fetch)
  channels.forEach((slug, index) => {
    fetch(`https://api.are.na/v2/channels/${slug}?per=1`)
      .then(res => res.json())
      .then(data => {
