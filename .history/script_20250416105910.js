...
function placeLinkedWord(text, linkHref = '', onClick = null, container = overlayGrid) {
  const spanCols = text.length;
  const { row, col } = nextFreeCell(spanCols, 1);

  const wrapper = document.createElement('div');
  wrapper.style.display = 'contents'; // Let grid work as-is

  for (let i = 0; i < text.length; i++) {
    const letterDiv = document.createElement('div');
    letterDiv.className = 'overlay-item';
    letterDiv.style.gridColumn = col + i;
    letterDiv.style.gridRow = row;

    const span = document.createElement('span');
    span.textContent = text[i];
    span.style.pointerEvents = 'none'; // Let parent handle interaction
  ...
  clickLayer.style.gridColumn = `${col} / span ${spanCols}`;
  clickLayer.style.gridRow = row;
...
placeLinkedWord('p, p text about etc', menuLink);
placeSpacer();
