...
function adjustGridRows(contentEl, totalCells) {
  ...
  const oldGrid = gridWrapper.querySelector('.grid-columns-rows');
  const newGrid = createGridStructure(rows * cols);
  gridWrapper.replaceChild(newGrid, oldGrid);
}
...
function createChannelItem(channelData, index) {
  ...
  const contentGrid = createGridStructure(200); // initial dummy size
