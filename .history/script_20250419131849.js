...
function createGridStructure(numItems) {
  ...
  const rows = Math.ceil(numItems / cols);  // Calculate the number of rows
  const totalCells = rows * cols;

  // Create the correct number of cells to fit the rows and columns
  for (let i = 0; i < totalCells; i++) {
    const cell = document.createElement('div');
    cell.className = 'grid-item';

    // Create sub-cells (optional for inner structure)
    for (let j = 0; j < 4; j++) {
      const sub = document.createElement('div');
      sub.className = 'sub-cell';
      cell.appendChild(sub);
  ...
  fetch(`https://api.are.na/v2/channels/${slug}?per=100`)
    ...
    .then(data => {
      ...
      if (data.metadata?.description) {
        ...
        totalItems++; // Count the description as one item
      }
      ...
      data.contents.forEach(block => {
        if (block.image?.display?.url) {
          ...
          img.style.gridRow = `span 5`;
          contentEl.appendChild(img);
          totalItems++; // Count the image as one item
        }

        const text = (block.title || block.content || '') + '\n';
        addTextBlock(text, contentEl);
        totalItems++; // Count the text block as one item
      });

      loadedChannels.add(slug);

      // Recalculate the grid rows based on the number of content items (including images)
      adjustGridRows(contentEl, totalItems);
    });
}
...
function adjustGridRows(contentEl, totalItems) {
  const rows = Math.ceil(totalItems / cols);  // Calculate the number of rows based on the total content items (text + images)

  // Remove all existing rows (children) and re-create the grid with the updated number of rows
  const gridWrapper = contentEl.closest('.grid-wrapper-inner');
  const grid = createGridStructure(totalItems); // Create new grid with updated row count
  gridWrapper.replaceChild(grid, gridWrapper.querySelector('.grid-columns-rows')); // Replace old grid with new one
}
...
function createChannelItem(channelData, index) {
  ...
  const grid = createGridStructure(title.length);  // Create grid for the channel name
