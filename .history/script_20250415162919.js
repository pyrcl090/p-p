...
function nextFreeCell(spanCols = 1, spanRows = 1) {
  while (true) {
    ...
    if (fits) {
      ...
      const position = { row: currentRow + 1, col: currentCol + 1 }; // CSS grid is 1-based
      currentCol += spanCols;
      return position;
    }
  ...
  .then(data => {
    ...
    blocks.forEach(block => {
      ...
      if (block.image && block.image.display && block.image.display.url) {
        ...
        const spanCols = Math.floor(Math.random() * 6) + 3; // 3 to 8 columns
        const spanRows = 5; // fixed to 5 rows tall
        ...
        img.style.opacity = '.8';
        img.style.alignContent = '.8';
      
        overlayGrid.appendChild(img);
      
        // ⬇️ Shift cursor below image to avoid overlap with text
        currentCol = 0;
