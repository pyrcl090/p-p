    ...
    .then(data => {
      ...
      data.contents.forEach(block => {
        if (block.image?.display?.url) {
          ...
          imgWrapper.style.gridColumn = 'span 15'; // Image wrapper spans 15 columns
          imgWrapper.style.height = 'calc(30px * 5)'; // Set the wrapper height to 5 rows

          const img = document.createElement('img');
          img.src = block.image.display.url;
          img.className = 'grid-image';

          // Set image's height and width properly
          img.style.width = '100%'; // Ensure the image spans full width of the wrapper
          img.style.height = '100%'; // Ensure image height stays within the wrapper's span

          // Randomly align the image (left, center, or right)
          const alignments = ['left', 'center', 'right'];
          const randomAlignment = alignments[Math.floor(Math.random() * alignments.length)];
          imgWrapper.style.textAlign = randomAlignment;

          imgWrapper.appendChild(img);
          contentEl.appendChild(imgWrapper);

          // After the image, we move to the next row
          totalCells += 15 * 5; // Image is 15 cols wide and 5 rows tall
        }
