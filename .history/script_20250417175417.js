...
const channels = [
  ...
  // Create the base grid (background with sub-cells)
  ...
  function createChannelName(title, slug, index) {
    ...
    // Add characters to the channel name
    for (const char of title) {
  ...
  // Create the channel content item
  ...
  // Place text blocks inside the content area
  function placeTextBlock(text, container) {
    for (const char of text) {
  ...
  // Setup grid and insert channel names/content
  ...
  // Initialize grid and channel setup
  createBaseGrid();
  setupChannels();
