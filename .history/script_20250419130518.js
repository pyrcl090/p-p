...
const cellSize = 30; // Cell size (for sizing purposes)
let currentlyOpenId = null;
const loadedChannels = new Set();
...
function createGridStructure(numItems) {
  ...
  const rows = Math.ceil(numItems / cols); // Calculate rows needed
...
function fillChannelContent(contentEl, slug) {
  ...
  fetch(`https://api.are.na/v2/channels/${slug}?per=100`)
    ...
    .then(data => {
      ...
      if (data.metadata?.description) {
        totalChars += data.metadata.description.length;
        totalItems++;
      }

      // Loop through contents to calculate text and image content
      data.contents.forEach(block => {
        if (block.image?.display?.url) {
          totalItems++;
        }
      ...
      // Total items to fit in grid
      const totalGridItems = totalChars + totalItems * cols;
      const newGrid = createGridStructure(totalGridItems);

      // Update the content grid behind the actual content
      const wrapper = contentEl.parentElement;
      wrapper.insertBefore(newGrid, contentEl);
      wrapper.removeChild(wrapper.firstChild);

      // Add the description text block if available
      if (data.metadata?.description) {
        addTextBlock(data.metadata.description, contentEl);
      }

      // Add all content blocks (text or images)
      data.contents.forEach(block => {
        if (block.image?.display?.url) {
          const img = document.createElement('img');
          img.src = block.image.display.url;
    ...
    .catch(error => console.error('Error loading channel content:', error));
}
...
function toggleContent(id, slug) {
  console.log(`Toggling content for: ${slug}`);
...
function createChannelItem(channelData, index) {
  ...
  contentItem.style.display = 'none';  // Content should be hidden initially

  const contentGrid = document.createElement('div');
  contentGrid.className = 'grid-columns-rows';
