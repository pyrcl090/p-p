...
const channels = [
  ...
  function placeText(text, container, onClick = null) {
  ...
  channels.forEach((slug, index) => {
    fetch(`https://api.are.na/v2/channels/${slug}?per=100`)
      ...
      .then(data => {
        ...
        placeText(data.title + '\n', overlayGrid, () => toggleChannelContent(toggleId));
        ...
        data.contents.forEach(block => {
          ...
          if (block.title || block.content) {
            const content = (block.title || block.content) + '\n';
            placeText(content, container);
          }
        });
      });
  });
  
  function toggleChannelContent(id) {
    const current = document.getElementById(id);
    if (!current) return;
  
    if (currentlyOpenId && currentlyOpenId !== id) {
      const previous = document.getElementById(currentlyOpenId);
