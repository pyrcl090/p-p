...
function placeSpacer() {
  console.log('Placing spacer');
  currentCol = 0;
  currentRow++;
}

function adjustGridToOverlay() {
  const height = overlayGrid.scrollHeight;
  console.log(`Overlay height: ${height}`);

  // Get the number of rows the grid needs to cover the content
  const neededRows = Math.ceil(height / 30);
  const extraNeeded = neededRows - rows;

  console.log(`Extra needed rows: ${extraNeeded}`);
...
// --- TITLE ---
placeText('p projects', false, '', overlayGrid, () => {
...
channels.forEach((slug, index) => {
  fetch(`https://api.are.na/v2/channels/${slug}?per=100`)
    .then(res => res.json())
    .then(data => {
      console.log(`Fetched data for channel ${data.title}`);

      const toggleId = `channel-content-${index}`;

      // Generate container FIRST and hide it
      const channelContainer = document.createElement('div');
      channelContainer.id = toggleId;
      channelContainer.style.display = 'none'; // Start hidden
      ...
      // Title in overlayGrid, clickable
      placeSpacer();
      placeText(`${data.title}`, false, '', overlayGrid, () => {
        console.log(`Channel clicked: ${data.title}`);

        const target = document.getElementById(toggleId);

        // Hide previous open
        if (currentlyOpenId && currentlyOpenId !== toggleId) {
          const prev = document.getElementById(currentlyOpenId);
          if (prev) {
            console.log(`Hiding previous channel content: ${currentlyOpenId}`);
        ...
        if (target) {
          ...
          console.log(`Toggling visibility of content. Current state: ${isVisible ? 'visible' : 'hidden'}`);
          target.style.display = isVisible ? 'none' : 'grid';
          currentlyOpenId = isVisible ? null : toggleId;

          // Expand the grid only when the content is shown
          if (!isVisible) {
            adjustGridToOverlay();
          }
        }
