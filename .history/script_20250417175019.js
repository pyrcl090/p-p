...
const channels = [
  ...
  const gridWrappers = document.querySelectorAll('.grid-wrapper'); // Now selects multiple grid-wrapper elements
  const cols = 15;
  const rows = 40;
  const cellSize = 30;
  let currentlyOpenId = null;
  ...
  // 👉 Create the background subgrid (with dashed sub-cells)
  function createSubgrid(gridContainer, rows = 40, cols = 15) {
    // Ensure the grid-border exists before creating items
    ...
    for (let i = 0; i < rows * cols; i++) {
      ...
      // Add sub-cells (for background effect or other styling purposes)
      for (let j = 0; j < 4; j++) {
        const sub = document.createElement('div');
        sub.className = 'sub-cell';
        cell.appendChild(sub);
  ...
  // 👉 Layout the channel title and clickable toggle
  function placeChannelTitle(channelData, channelIndex, gridBorder) {
    const title = channelData.title || `Channel ${channelIndex + 1}`;
    const wrapper = createChannelContainer(channelIndex);
    const toggleClass = `channel-content-${channelIndex}`;
  
    for (const char of title) {
      const item = createGridItem(char);
      item.style.cursor = 'pointer';
      item.style.pointerEvents = 'auto';
      item.addEventListener('click', () => toggleChannel(toggleClass, channelData.slug, wrapper, gridBorder));
      wrapper.appendChild(item);
    }
    ...
    const contentContainer = document.createElement('div');
    contentContainer.classList.add(toggleClass);
    contentContainer.classList.add('channel-content');
    contentContainer.style.gridColumn = `1 / span ${cols}`;
    contentContainer.style.display = 'none';
    contentContainer.style.gridTemplateColumns = `repeat(${cols}, ${cellSize}px)`;
    ...
    wrapper.appendChild(contentContainer);
    gridBorder.appendChild(wrapper);
  }
  ...
  // 👉 Toggle a channel content block open/closed
  function toggleChannel(toggleClass, slug, wrapper, gridBorder) {
    // Close the previous open content
    if (currentlyOpenId && currentlyOpenId !== toggleClass) {
      const prev = gridBorder.querySelector(`.${currentlyOpenId}`);
      if (prev) prev.style.display = 'none';
    }
  
    const contentEl = gridBorder.querySelector(`.${toggleClass}`);
    const isOpen = contentEl.style.display === 'grid';
  ...
  gridWrappers.forEach((gridWrapper) => {
    gridWrapper.querySelectorAll('.grid-border').forEach((gridBorder) => {
      channels.forEach((slug, index) => {
        fetch(`https://api.are.na/v2/channels/${slug}?per=1`)
          .then(res => res.json())
          .then(data => {
            data.slug = slug;
            placeChannelTitle(data, index, gridBorder);
          });
      });
    });
  });
