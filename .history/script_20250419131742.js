...
function fillChannelContent(contentEl, slug) {
  fetch(`https://api.are.na/v2/channels/${slug}?per=100`)
    ...
    .then(data => {
      ...
      // Recalculate the grid rows based on the number of content items
      adjustGridRows(contentEl);
    });
}
...
function adjustGridRows(contentEl) {
  const totalItems = contentEl.children.length;
