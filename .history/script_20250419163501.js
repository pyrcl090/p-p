...
function fillChannelContent(contentEl, slug) {
  ...
  fetch(`https://api.are.na/v2/channels/${slug}?per=100`)
    ...
    .then(data => {
      ...
      data.contents.forEach((block, idx) => {
        console.log(`Processing block #${idx} - Class: ${block.class}`);

        // --- IMAGE BLOCK ---
        if (block.class === 'Image' && block.image?.display?.url) {
          console.log('Image URL:', block.image.display.url);
          const img = document.createElement('img');
          img.src = block.image.display.url;
          img.className = 'grid-image';
          ...
          const imageText = block.title || block.description;
          if (imageText) {
            console.log('Image description:', imageText);
            addTextBlock(imageText + '\n', contentEl);
            totalCells += imageText.length;
            totalCells += (cols - (imageText.length % cols)) % cols;
          }
        }

        // --- TEXT BLOCK ---
        else if (block.class === 'Text' && block.content) {
          console.log('Text block content:', block.content);
          addTextBlock(block.content + '\n', contentEl);
          totalCells += block.content.length;
          totalCells += (cols - (block.content.length % cols)) % cols;
        }

        // --- FALLBACK FOR OTHER BLOCKS ---
        else {
          const fallbackText = block.title || block.content || block.description;
          if (fallbackText) {
            console.log('Fallback block text:', fallbackText);
            addTextBlock(fallbackText + '\n', contentEl);
            totalCells += fallbackText.length;
            totalCells += (cols - (fallbackText.length % cols)) % cols;
          } else {
            console.log(`Block #${idx} had no usable text.`);
          }
        }
      });
      ...
      loadedChannels.add(slug);
      console.log(`Finished loading channel: ${slug}`);
    })
    .catch(err => {
