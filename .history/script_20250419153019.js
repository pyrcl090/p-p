...
function addTextBlock(text, container) {
  ...
  addSeparator(container); // space after text block
}
    ...
    .then(data => {
      ...
      data.contents.forEach(block => {
        ...
        if (text.trim()) {
          ...
          totalCells += text.length + cols; // +cols for separator
        }

        if (block.image?.display?.url) {
          const colSpan = Math.floor(Math.random() * 6) + 3; // 3–8 columns
          const rowSpan = 5; // max 5 rows tall
          ...
          totalCells += cols; // for spacer
        }
      });

      console.log(`Content added for channel: ${slug}`);

      const contentGrid = contentEl.previousElementSibling; // the background grid
      ...
      contentEl.parentElement.replaceChild(newGrid, contentGrid);
...
function createChannelItem(channelData, index) {
  ...
  const contentGrid = createGridStructure(200); // default
