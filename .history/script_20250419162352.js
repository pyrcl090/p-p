    ...
    .then(data => {
      ...
      data.contents.forEach(block => {
        if (block.class === 'Image' && block.image?.display?.url) {
          console.log('Adding image block:', block.image.display.url);
          const img = document.createElement('img');
          img.src = block.image.display.url;
          img.className = 'grid-image';
          ...
          // Randomly align image left, center, or right
        ...
        if (block.class === 'Text' && block.content) {
          console.log('Adding text block:', block.content);
          addTextBlock(block.content, contentEl);
          totalCells += block.content.length;
          totalCells += (cols - (block.content.length % cols)) % cols;
        }

        const fallbackText = block.title || block.content;
        if (!['Image', 'Text'].includes(block.class) && fallbackText) {
          console.log('Adding fallback block:', fallbackText);
          addTextBlock(fallbackText, contentEl);
