...
const cols = 15; // Number of columns in the grid
...
function createGridStructure(numItems) {
  ...
  const rows = Math.ceil(numItems / cols); // Calculate the number of rows required
...
function fillChannelContent(contentEl, slug) {
  ...
  fetch(`https://api.are.na/v2/channels/${slug}?per=100`)
    ...
    .then(data => {
      ...
      // Update grid height based on the content added
      adjustGridHeight(contentEl);
...
function adjustGridHeight(contentEl) {
  const gridWrapper = contentEl.closest('.grid-wrapper-inner');
  const gridItems = gridWrapper.querySelectorAll('.grid-item');

  const totalHeight = gridItems.length * cellSize; // Calculate total height based on grid items

  // Log before and after grid height adjustment
  console.log(`Before adjustment, grid height: ${gridWrapper.style.height}`);
  console.log(`Calculated total height: ${totalHeight}px`);

  // Set the height of the grid wrapper dynamically
  gridWrapper.style.height = `${totalHeight}px`;

  console.log(`After adjustment, grid height: ${gridWrapper.style.height}`);
}
